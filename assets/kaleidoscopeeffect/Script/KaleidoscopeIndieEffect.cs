...
public class KaleidoscopeIndieEffect : MonoBehaviour {
	...
	protected Material material {
		get {
			if (m_Material == null) {
				m_Material = new Material( "Shader \"Kaleidoscope/texture\" {" +
				                           "Properties {"+
											"_MainTex (\"Base (RGB)\", 2D) = \"white\" {}"+
											"}"+
				                           "SubShader { Pass { " +
				                           "    Lighting Off ZTest Off ZWrite Off Cull Off Fog { Mode Off } " +
	...
	protected Material material2 {
		get {
			if (m_Material2 == null) {
				m_Material2 = new Material( "Shader \"Kaleidoscope/color\" {" +
				                           "SubShader { Pass { " +
