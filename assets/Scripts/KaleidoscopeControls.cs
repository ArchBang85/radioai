...
public class KaleidoscopeControls : MonoBehaviour {
    ...
    //public int[] BTManager.instance.digitalPinValues = new int[7];
    //public int[] BTManager.instance.analogPinValues = new int[3];
    ...
    private int[] rightSideSwitch = new int[2];
    ...
    private float RGBchangetimer = 0.3f;
	...
	void Start () {
        ...
        rightSideSwitch[0] = BTManager.instance.digitalPinValues[3];
        rightSideSwitch[1] =  BTManager.instance.digitalPinValues[4];
	...
	void radioBoxController(){
		...
		if(radioBoxControls)
		{
			...
			var mainDial = BTManager.instance.analogPinValues[0];

			rotationSpeed = mainDial / 4096f * 15f - 7.5f;
            Debug.Log("main dial: " + mainDial + " converted: " + rotationSpeed);
			...
			// 4 * -5 / 4092 + y = 5 when y = 
			var leftDial = BTManager.instance.analogPinValues[1]; // min 4096 half 3700 max 4
            //lightningIntensity = 4096 * -5 / 4092 + leftDial;
            lightningIntensity = leftDial / 4096f * 5f;

			// Colour switch
            if (BTManager.instance.digitalPinValues[3] != rightSideSwitch[0] || BTManager.instance.digitalPinValues[4] != rightSideSwitch[1])
            { // Need to understand when the change happens...
                rightSideSwitch[0] = BTManager.instance.digitalPinValues[3];
                rightSideSwitch[1] = BTManager.instance.digitalPinValues[4];
                float TimerReset = 0.5f;
                if (BTManager.instance.digitalPinValues[3] == 0 && BTManager.instance.digitalPinValues[4] == 1)
                {
                    // Auto
                    RGBswitch = 0;
                    RGBchangetimer = TimerReset;

                }
                else if (BTManager.instance.digitalPinValues[3] == 1 && BTManager.instance.digitalPinValues[4] == 1)
                {
                    // Buzzer
                    RGBswitch = 1;
                    RGBchangetimer = TimerReset;
                }
                else if (BTManager.instance.digitalPinValues[3] == 1 && BTManager.instance.digitalPinValues[4] == 0)
                {
                    // Radio on
                    RGBswitch = 2;
                    RGBchangetimer = TimerReset;
            ...
            if (RGBchangetimer > 0) RGBchangetimer -= Time.deltaTime;
            ...
            if(RGBchangetimer < 0)
            { 
                float rightDial = BTManager.instance.analogPinValues[2];

                rightDial = rightDial / 4096f * 255;  //rightDial * (255/4093f) + (-3 * 255 / 4093f);

                // How do we make the colours a bit sticky when you switch the dial?

			    // Make surightDialightDiale it isn't out of bounds, just in case:
			    if (rightDial < 0) rightDial = 10;
			    if (rightDial > 255) rightDial = 245;

			    colourRGB[RGBswitch] = (int)rightDial;
            }
			// Colour size
			...
			if(BTManager.instance.digitalPinValues[6] == 0 && snoozePressed < 0 || Input.GetKeyDown ("r"))
			{
			...
			if(BTManager.instance.digitalPinValues[2] == 1)
			{
				showText = false;
			} else {
				showText = true;
			...
			if(BTManager.instance.digitalPinValues[0] == 0 && BTManager.instance.digitalPinValues[1] == 1)
			{
				// FM
				this.gameObject.GetComponent<TwirlEffect>().enabled = false;
				this.gameObject.GetComponent<VortexEffect>().enabled = true;
			} else if (BTManager.instance.digitalPinValues[0] == 1 && BTManager.instance.digitalPinValues[1] == 1){
				// LW - Standard view
				this.gameObject.GetComponent<TwirlEffect>().enabled = false;
				this.gameObject.GetComponent<VortexEffect>().enabled = false;

			} else if (BTManager.instance.digitalPinValues[0] == 1 && BTManager.instance.digitalPinValues[1] == 0){
				// AM
				// Set twirl view
	...
	void TargetProximity()
	{
		...
		if(!leapmotionControls)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				proximity[i] += Mathf.Abs (lightningIntensityTargets[i] - lightningIntensity) / 1.5f;
				...
				colourProximity = colourProximity / 30;
